...
This style guide outlines the best practices and conventions for creating clear, consistent, and high-quality documentation for our project. By following these guidelines, we can ensure a cohesive writing style and tone across all documentation.

The guide aims to provide guidelines for writing clear, concise, and consistent documentation. It covers language, tone, formatting, and other aspects of documentation.
...
:::{important}
As specified below, use colons (i.e., `:::`) instead of the typical hyphens when opening and closing blocks.
:::
...
Use EST and specify this when the date or time is used (e.g., `September 9, 2023, at 9-5 a.m. ET`).
:::
The writing style in documentation is critical for ensuring consistency and understanding. Here are some best practices:

**Clarity**: Be as clear and straightforward as possible. Avoid ambiguity, complex sentences, and unusual words. **KEEP IT SIMPLE.**
...
##  Formatting
MyST allows for markdown for our Sphinx project (i.e., [RTD]).
...
:::::{important}
We use the [`colon_fence` extension] rendered by MyST to parse three colons (i.e., *:*) to open and close blocks.

Optional arguments specified for a directive are listed in [MyST Spec].

::::{note}
[MyST-Parser Documentation] includes several constructs we can use to enhance our pages. **Use elements with purpose, whether to call attention, reference related content, or warning.
::::

Nesting blocks, along with the ability to set parameters inline.

::::{note}
There is no need to indent source text, as nested directives are explicitly clear by the number of `:` characters used to open and close.
::::

::::{important}

**Do not add line breaks unless a line break is intended for the rendered view.** Each paragraph remains on the same line, with line breaks used to clearly show transitions between paragraphs, sections, and directives.
...
:::{note}
Add line breaks before and after the opening and closing of directives (i.e., outside the structure). Only add a single line break after the opening, which is the case for most directives; for those that do not require any inputs but titles (e.g., `:::{directive} Title`), omit line breaks after the opening.
:::
::::

::::{important}
These nested admonitions were done mostly to demonstrate how it can be done. There are exceptions, but this is quite nested; be sure to consider if the flow of information is done most effectively when nests are deeper, or even as deep as this example (i.e., five colons `:::::` deep).
::::
:::::
...
::::{grid-item-card}
:::{important}
- Use to emphasize a sub-topic or sub-point.
- Use to emphasize a specific point.
- Please feel free to use it when extra care is needed for a specific task.
- Use to warn the user of expected behaviors or outputs.
:::
::::
...
::::{grid-item-card}
:::{seealso}
Use to reference another section, page, or reference that extends, provides details on a subtopic or step, or serves as supplemental.
:::
::::
...
### Tabs
[MyST Tabs] are a great way to present material (see {ref} GPUs for Deep Learning). These must be used properly: in situations where the same task or topic spans multiple setups or interfaces. For instance, installing deep learning to run on the GPUs is the task in the aforementioned example, where the tab chosen is either PyTorch or TensorFlow.
...
- **Jargon**: Avoid using jargon unless necessary. If you have to use it, please define it first.
- **Language**: Use either American or British English consistently throughout your documentation.

## Documentation Structure
- Organize documentation into folders representing sections, with each file as an item of its section.
- Use a clear hierarchy for headings and subheadings.
  - A single level-one heading (i.e., `# HEADING`) at the top of the page: `HEADING` will appear in the table of contents.
   - Add file paths to [index.md] as follows:
:::{code-block}
```{toctree}
:hidden:
:caption: section
...
```
:::
Where *section* is the top-level index listed in the table of contents and *filename* is the markdown file. Note that `.md` can be omitted. Furthermore, when naming a file or directory, use all lowercase with no spaces. For example, {ref}`home-directory-storage-quota` is named `homequota.md`.
...
- Descriptive Links: Avoid using phrases like "click here" for hyperlink text. Instead, you can use descriptive text that informs the reader about the content they'll find when they follow the link.
...
Markdown and MyST provide a simple way to format your content. Here are some tips for using this syntax effectively:
- **Basic Markdown**: Familiarize yourself with basic Markdown syntax, including headers, lists, links, images, and code blocks.
- **MyST Features**: MyST extends the standard Markdown syntax with additional features such as cross-references, footnotes, directives, warnings, and other particular elements. And please make sure you use these features correctly.
...
:::{note}
There are many [admonitions] to choose from. Please use your best judgment, look at other examples, and use the appropriate tone.
:::
...
##  Review Process
Finally, please ensure your documentation goes through a thorough review process before publication. We do this through PRs on GitHub.

- **Peer Review**: Encourage team members to review each other's work. This can help catch errors, ambiguities, and inconsistencies that the original writer may have yet to notice.
- **Proofreading**: Check for spelling, grammar, and punctuation errors. Tools like Grammarly or Language Tool can help with this.

:::{tip}
Consider using Notion to draft when adding a substantial amount of content, whether a new page or several sections. Then, update the RTD pages (i.e., this project) for a technical review. This way, development can be more interactive, time is not wasted adjusting formatting and other syntax, and the history is much simpler (and more promising) than our own managed via Git.
:::
...
:::{important}
All members of the RC are welcome to contribute to the documentation. We encourage you and greatly appreciate it. Please don't hesitate to ask for tasks or make suggestions to improve our *User Facing Documentation*.
:::
