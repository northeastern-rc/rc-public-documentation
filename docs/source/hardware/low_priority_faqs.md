...
A job submitted to the `lowpriority` partition that comes from a user **who is NOT** a member of the private partition on which the job is scheduled to run. If a member of the private partition submits a job to the private partition, the "low priority" job will be terminated from the `lowpriority` partition and re-queued.
::::
...
Please keep in mind that jobs submitted by private partition members to their own partition **always** have higher priority.
::::
